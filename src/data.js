export const data = {
  ...
  experience: [
    {
      ...
      achievements: [
        "Samsung Pay 글로벌 및 한국 서비스 결제/리워드/로열티 핵심 시스템 아키텍처 설계 및 구축",
        "수백만 트래픽 규모의 레거시 시스템을 독립적이고 확장 가능한 마이크로서비스(MSA) 구조로 성공적 분리/전환",
        "CI/CD 파이프라인 전면 도입, Airflow 기반 데이터 파이프라인 구축 및 인프라 클라우드(AWS/GCP) 이전"
      ]
    }
  ],
  projects: [
  ...
  troubleshooting: [
    ...
    },
    {
      ...
      result: "배치 서버 리소스 운영 리스크 완벽 감소, 재구축 과정 중 상세한 문서화를 통해 운영 인수인계 및 히스토리 레퍼런스 확보."
    },
    {
      id: "admin-portal",
